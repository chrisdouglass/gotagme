    ...
    router.route('/:tagID?')
        ...
        .post(
            ...
            (req: Request, res: Response, next: NextFunction) => {
              this._photoAPI.handleModifyTag(
                req.params.tagID, huskysoft.gotagme.tag.ModifyTagRequest.fromObject(req.body),
                req.user as User).then((response: huskysoft.gotagme.tag.GetTagsResponse) => {
                  res.json(response);
                }).catch(next);
            })
        .delete(
