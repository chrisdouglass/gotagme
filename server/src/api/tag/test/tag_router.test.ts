...
export class TagRouterTest extends RouterTest {
  ...
  async getTagCounts() {
    ...
    for (let i = 0; i < expectedCount; i++) {
      ...
      await this.tagStore.addCostumeTagToPhoto(taggedCostume1, photo, this.loggedIn);
      await this.tagStore.addCostumeTagToPhoto(taggedCostume2, photo, this.loggedIn);
    ...
    const userRequest: huskysoft.gotagme.tag.GetTagCountsRequest = new huskysoft.gotagme.tag.GetTagCountsRequest({
      userIDs: [taggedUser1.userID, taggedUser2.userID],
    });
    const userRes: request.Response =
        await request(this.app).post('/counts')
            .set('Content-Type', 'application/json')
            .send(userRequest.toJSON()).expect(200).expect('Content-Type', /json/);
    ...
    userResponse.responses.map((result: huskysoft.gotagme.tag.IGetTagCountResponse) => {
      result.count!.should.equal(5);
    });

    const costumeRequest: huskysoft.gotagme.tag.GetTagCountsRequest = new huskysoft.gotagme.tag.GetTagCountsRequest({
      costumeIDs: [taggedCostume1.costumeID, taggedCostume2.costumeID],
    });
    const costumeRes: request.Response =
        await request(this.app).post('/counts')
            .set('Content-Type', 'application/json')
            .send(costumeRequest.toJSON()).expect(200).expect('Content-Type', /json/);
    ...
    costumeResponse.responses.map((result: huskysoft.gotagme.tag.IGetTagCountResponse) => {
      result.count!.should.equal(5);
    });

    const hashtagRequest: huskysoft.gotagme.tag.GetTagCountsRequest = new huskysoft.gotagme.tag.GetTagCountsRequest({
      hashtags: [hashtag1, hashtag2],
    });
    const hashtagRes: request.Response =
        await request(this.app).post('/counts')
            .set('Content-Type', 'application/json')
            .send(hashtagRequest.toJSON()).expect(200).expect('Content-Type', /json/);
    ...
    hashtagResponse.responses.map((result: huskysoft.gotagme.tag.IGetTagCountResponse) => {
      result.count!.should.equal(5);
    });
  }
}
